classdef MRMOTUS_Operator
    ...
    methods
        ...
        function obj = MRMOTUS_Operator(ReferenceImage,KspaceTrajectory,param_struct)
            ...
            %   KspaceTrajectory        - Kspace trajectory for the forward model [#readout_samples x d x interleaves], should be scaled between [ImDim(1)/2 ImDim(2)/2 ImDim(3)/2]
            ...
            obj.ImDims              	= max(abs(obj.KspaceTrajectory(:,spatial_ordering(1:obj.NumberOfSpatialDims),:)),[],[1,3])*2;
            disp(['+ Extracted ImDims from ReferenceImage: ',num2str(obj.ImDims)]);
            obj                         = obj.set_reference_image(ReferenceImage(:));
           
%             obj.ImDims                  = size(ReferenceImage); %round(numel(ReferenceImage(:,1))^(1/obj.NumberOfSpatialDims));
            obj.ReferenceGrid           = MRMOTUS_Operator.MakeReferenceGrid(obj.ImDims,obj.NumberOfSpatialDims);
            
            
%             ReferenceImage = ReferenceImage(:);

            if ~isfield(obj.param_struct,'motionfieldoperator_type')
                obj.param_struct.motionfieldoperator_type = 'finufft';
            end
...
%             Phi_rshp                  = Phi_rshp                    * Phi_scaling;
            ...
            % - Size: N^d x d*R
            GradientPhi                 = zeros([prod(obj.ImDims),obj.NumberOfSpatialDims*obj.param_struct.NumberOfComponents]);  
...
%                 spe = speye(size(PreconditionMatrix{1},1),size(PreconditionMatrix{1},1));
                ...
                parfor dynamic = 1:obj.NumberOfDynamics % some notes: matlab transfers all variables in the loop, regardless of whether they are accessed or within if statements
                    ...
                    if 1==1 %RegularizationFlag && parallel_reg
...
%                 end
            ...
            if obj.param_struct.VisualizationFlag
                for rr=1:obj.param_struct.NumberOfComponents
                    viss = reshape(Phi(:,rr),[obj.ImDims,obj.NumberOfSpatialDims,1]);
...
%             basis_options_spatial.spatial_ordering  = [2 1 3 4];
            basis_options_spatial.spatial_ordering  = [2 1 3 4];
            basis_options_spatial.N                 = obj.ImDims(basis_options_spatial.spatial_ordering(1:obj.NumberOfSpatialDims));
...
%                     obj.RegularizationOptions.Types.Determinant.A_2         = [obj.RegularizationOptions.Types.Determinant.A_1(:,end/2+1:end),obj.RegularizationOptions.Types.Determinant.A_1(:,1:end/2)];
        ...
        function obj=initialize_dcf(obj)
            ...
            if ~isfield(obj.param_struct,'dcf') || isempty(obj.param_struct.dcf)
                ...
                dcf_coords = dcf_coords./max(abs(dcf_coords),[],[1 3]);
                dcf_coords(:,3,:)=zeros(size(dcf_coords(:,1,:)));
                obj.DCF = reshape(iterative_dcf_estimation(dcf_coords*obj.ImDims(1),6),[],size(obj.KspaceTrajectory,3));
    ...
    methods(Static)
        ...
        function [ObjfuncValDynamic,GradientDynamic,ForwardSignal,DVF_Operator] = forward_and_gradient_singledynamic(MotionField,KspaceTrajectory,KspaceData,PrecondMatrix,static_transfer_struct,ParallelComputationFlag,varargin) % should be faster paralellized than separately forward and adjoint
            ...
            DVF_Operator                = MotionFieldOperator(KspaceTrajectory,static_transfer_struct.ReferenceGrid(static_transfer_struct.referenceImageMask(:),:),MotionField(static_transfer_struct.referenceImageMask(:),:),'finufft_cartesian'); % forward: maps motionfields to data, adjoint: maps data to motionfields
...
%                 size(static_transfer_struct.ReferenceImage)
%                 size(static_transfer_struct.referenceImageMask(:))
...
%                         [regularizationEnergy,regularizationGradient]   = vectorial_tv_operator(MotionField,RegularizationOptions.Types.(reg_types{i}).Eps,RegularizationOptions.Types.(reg_types{i}).D.Value);
                        [regularizationEnergy,regularizationGradient]   = tv_vector_operator_coefficients(MotionField,RegularizationOptions.Types.(reg_types{i}).Eps,static_struct.ImDims,RegularizationOptions.GridSpacing,RegularizationOptions.Types.(reg_types{i}).D.Value);
        ...
        function RefGrid=MakeReferenceGrid(N,d,affine_flag)
            ...
            if d==3 % 3D
                [tmp_x,tmp_y,tmp_z]     = meshgrid(-N(2)/2:N(2)/2-1,-N(1)/2:N(1)/2-1,-N(3)/2:N(3)/2-1);
                RefGrid                 = [tmp_x(:),tmp_y(:),tmp_z(:)];
                clearvars tmp_x tmp_y tmp_z
            elseif d==2 % 2D
                [tmp_x,tmp_y]           = meshgrid(-N(2)/2:N(2)/2-1,-N(1)/2:N(1)/2-1);
